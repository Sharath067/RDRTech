...
} from '../apiServices/apiServices'; // Add getSubscriptionPlans import
...
const CreatingSubscriptionPlans = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState('');
  const [existingPlans, setExistingPlans] = useState([]); // Store existing plans

  // Load existing plans on component mount
  useEffect(() => {
    const loadExistingPlans = async () => {
      try {
        const plans = await getSubscriptionPlans(); // Fetch existing plans
  ...
  const checkDuplicatePlan = planName => {
    return existingPlans.some(
      plan => plan.name && plan.name.toLowerCase() === planName.toLowerCase(),
    );
  };
  ...
  const handlePlanSelect = e => {
    ...
    // Check for duplicate plan before setting form data
    if (selectedPlan && checkDuplicatePlan(selectedPlan)) {
      toast.error(
        `${selectedPlan} plan already exists. Duplicate plans cannot be created.`,
      );
  ...
  const submitPlan = async postData => {
    try {
      const newPlan = await createSubscriptionPlan(postData);
      toast.success('Plan created successfully!');

      // Update existing plans list with the new plan
      setExistingPlans(prevPlans => [...prevPlans, newPlan]);
      ...
      // Check if error is specifically about duplicate plan
      if (
        (error.message && error.message.includes('duplicate')) ||
        error.message.includes('already exists')
      ) {
        toast.error(
          'This plan already exists. Duplicate plans cannot be created.',
        );
      } else {
        toast.error('Failed to create plan. Please try again.');
      }
  ...
  const handleSubmit = async e => {
    ...
    // Double-check for duplicate plan before submission
    if (checkDuplicatePlan(formData.planName)) {
      toast.error(
        `${formData.planName} plan already exists. Duplicate plans cannot be created.`,
      );
      return;
    }
