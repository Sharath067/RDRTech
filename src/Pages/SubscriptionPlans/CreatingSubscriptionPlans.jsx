...
import {createSubscriptionPlan, getAllPlans} from '../apiServices/apiServices';
...
const CreatingSubscriptionPlans = () => {
  ...
  useEffect(() => {
    const loadExistingPlans = async () => {
      try {
        const plans = await getAllPlans(); // Fetch existing plans
  ...
  const handlePlanSelect = e => {
    ...
    if (selectedPlan && checkDuplicatePlan(selectedPlan)) {
      toast.error(`${selectedPlan} plan is already exists.`);
  ...
  const submitPlan = async postData => {
    try {
      ...
      if (
        ...
        toast.error('Free plan is already exists.');
      } else {
  ...
  const handleSubmit = async e => {
    ...
    if (checkDuplicatePlan(formData.planName)) {
      toast.error(`${formData.planName} plan is already exists.`);
      return;
    }
